...
impl CompanyService {
    ...
    pub async fn create(&self, request: CreateCompanyRequest) -> Result<Company, AppError> {
        ...
        let mut active = companies_entity::ActiveModel {
            ...
            active: Set(true),
            is_demo_mode: Set(request.is_demo_mode.unwrap_or(false)),
            segment: Set(request.segment),
            tax_regime: Set(request.tax_regime),
            max_users: Set(request.max_users),
            storage_limit_mb: Set(request.storage_limit_mb),
            notes: Set(request.notes),
            opening_date: Set(request.opening_date),
